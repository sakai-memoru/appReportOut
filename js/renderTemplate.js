...
(function () {
    ...
    let form_folder_param = '';
    let form_file_param = '';
    let data_folder_param = '';
    let data_filelike_param = '';
    let merged_folder_param = '';
    let merged_file_base_param = '';
    // - private method ---------------------------------------//
    ...
    let mergeTemplate = (templ_path, datafiles) => {
        // ## renderTEmplate.mergeTemplate
        const CONS_MODULE_NAME = "renderTEmplate.mergeTemplate";
        let ret = '';
        try {
            // - get paths
            let templPathAry = path.dirname(templ_path).split(path.sep);
            let sub_folder = templPathAry[templPathAry.length - 1];
            console.log(`sub_folder = ${sub_folder}`);
            //
            let merged_folder_def = path.join(base_folder_param, merged_folder_param);
            let merged_folder = path.join(merged_folder_def, sub_folder);
            ensureDirSync(merged_folder);
            let merged_file_base = path.join(merged_folder, merged_file_base_param);
            // console.log(`merged file = ${merged_file}`);
            //
            // - get contents
            console.log(`templ_path = ${templ_path}`);
            let templ_form = fs.readFileSync(templ_path, "utf-8");
            //console.log(templ_form);
            //
            const template = Handlebars.compile(templ_form);
            //
            // - (())-loop in data files
            let merged_data;
            let merged_str;
            let merged_file;
            datafiles.forEach((data_path, idx) => {
    ...
    let doProcess = () => {
        ...
        console.info('//------------------/ start !');
        // - generate pathes
        let form_folder = path.join(base_folder_param, form_folder_param);
        let form_file = path.join(form_folder, form_file_param);
        console.log(`form_file = ${form_file}`);
        //
        let data_folder = path.join(base_folder_param, data_folder_param);
        ...
        try {
            // - get files with glob
            let aryfiles = globby.sync(data_filelike);
