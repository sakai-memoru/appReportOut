...
let target_folder_param :string = '';
let target_file_param :string = '';
let output_folder_param :string = '';
...
let transformHtml = (file_path) :string => {
    // ## renderTEmplate.mergeTemplate
    const CONS_MODULE_NAME = "renderTEmplate.mergeTemplate";
    let ret:string = '';
    try{
        // - get html contents
        let html_contents :string = fs.readFileSync(file_path); // FIXME how to read a file of shift_jis
...
let doProcess =  () :any => {
    ...
    console.info('//------------------/ start !')
    // - generate pathes
    let target_folder :string = path.join(base_folder_param,target_folder_param);
    let target_file :string = path.join(target_folder,target_file_param);
    console.log(`target file = ${target_file}`);
    //
    let output_folder_def :string = path.join(base_folder_param, output_folder_param);
    let targetPathAry :string[] = path.dirname(target_folder).split(path.sep)
    let sub_folder :string = targetPathAry[targetPathAry.length-1]
    console.log(`sub_folder = ${sub_folder}`);
    let output_folder :string = path.join(output_folder_def, sub_folder);
    ensureDirSync(output_folder);
    let output_file :string = path.join(output_folder,"templ.table.html");
    console.log(`output file = ${output_file}`);
    //
    try {
        // - get output_content
        let output_contents :string = transformHtml(target_file);
